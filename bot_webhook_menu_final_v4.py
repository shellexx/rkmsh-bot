...
def message_handler(update, context):
    ...
    if step == "name":
        user["name"] = msg
        user["step"] = "age"
        keyboard = [[InlineKeyboardButton(str(i), callback_data=f"age_{i}")] for i in range(5, 18)]
...
def handle_age(update, context):
    ...
    lang = get_lang(chat_id)
    # Удобные кнопки классов: 1-5, 6-11 (2 ряда)
    class_buttons = [
        [InlineKeyboardButton(str(i), callback_data="class") for i in range(1, 6)],
        [InlineKeyboardButton(str(i), callback_data="class") for i in range(6, 12)]
    ]
    context.bot.send_message(chat_id=chat_id, text=TEXT[lang]["class"], reply_markup=InlineKeyboardMarkup(class_buttons))
...
"""
    try:
        ...
        with smtplylib.SMTP("smtp.mail.ru", 587) as server:
            server.starttls()
...
print("✅ Webhook установлен. Финальная версия готова.")
